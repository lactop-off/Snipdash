...
Read `docs/work/<feature>/要件定義.md`, `CLAUDE.md`, and `docs/AI開発プロセス.md` first. Match the existing visual language — study `apps/desktop/src/styles.css` (tokens: `--panel`, `--panel-2`, `--accent`, `--border`, `--radius`) and the card/tab/`card-action` patterns. Stay consistent; do not introduce a new look without reason.

## Your job (基本設計)
Write `docs/work/<feature>/基本設計.md` with:
...
Design only what the requirements ask; call out any requirement that is visually ambiguous. If QA or the Tech Lead returns **NG**, revise `基本設計.md` and address each point explicitly (note what changed).

Output: write `基本設計.md`, return a summary + path. **Do not edit code.**
